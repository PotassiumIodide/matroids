...
from matroids.construct.open_sets import (
...
@pytest.mark.parametrize('nulity_matroid, expected', [
    ...
    (( {1,2,3}, lambda X: len(X) if X <= {3} else len(X) - 1 ), [set(),{1,2}]                                 ),
    ...
    (( {1,2,3}, lambda X: len(X) - 2 if X == {1,2,3} else 0 ) , [set(),{1,2},{1,3},{2,3},{1,2,3}]             ),
    (( {1,2,3}, lambda X: 0 )                                 , [set(),{1},{2},{3},{1,2},{1,3},{2,3},{1,2,3}] ),
])
def test_from_nulity_matroid(nulity_matroid, expected):
    Os1 = from_nulity_matroid(nulity_matroid)
    Os2 = expected
    assert all(map(lambda O1: O1 in Os2, Os1)) and all(map(lambda O2: O2 in Os1, Os2))
